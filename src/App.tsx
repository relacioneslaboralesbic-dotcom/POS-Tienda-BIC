...
import { 
  ...
  BadgeInfo, Clock, UserCircle, ShieldCheck, FileDown, FileUp
} from 'lucide-react';
...
const CATEGORIES = ['Todos', 'Escritura', 'Papelería', 'Accesorios'];

// Inyección de estilos globales para la tipografía
const globalStyles = `
...
const LogoBIC = ({ size = "normal", showText = true }) => (
  <div className="flex items-center gap-3">
    ...
    <div className="hidden relative items-center justify-center">
      <div className={`bg-[${COLORS.bicOrange}] ${size === 'large' ? 'w-24 h-16' : 'w-12 h-8'} rounded-[50%] flex items-center justify-center border-2 border-black rotate-[-5deg]`}>
...
const App = () => {
  ...
  const [appMode, setAppMode] = useState('selection'); // 'selection', 'login_admin', 'login_employee', 'admin', 'employee'
  const [adminView, setAdminView] = useState('dashboard'); // 'dashboard', 'orders', 'inventory', 'history'
  ...
  const [isLoading, setIsLoading] = useState(true); // Para mostrar carga al iniciar
  
  // Efecto que se ejecuta al abrir la aplicación para cargar el inventario
  useEffect(() => {
    // 1. Aquí usamos tu API URL para descargar los datos del Excel
    fetch('https://sheetdb.io/api/v1/a174kd16wc31r') 
      .then(response => response.json())
      .then(data => {
        // 2. Formateamos los datos para asegurar que los números sean números
        const formattedData = data.map(p => ({
          ...p,
          id: parseInt(p.id) || Date.now() + Math.random(), // Si no hay ID, creamos uno
  ...
  // Función para actualizar el stock directamente en el Excel cuando se aprueba una venta
  const updateStockInSheet = async (id, newStock) => {
    try {
      await fetch(`https://sheetdb.io/api/v1/a174kd16wc31r/id/${id}`, {
        method: 'PATCH',
  ...
  const downloadCSVTemplate = () => {
    const csv = "codigo,nombre,precio,stock,categoria\nBIC-EX01,Articulo de Ejemplo,15.50,100,Escritura\n";
    const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
  ...
  const handleCSVUpload = (e) => {
    ...
    reader.onload = (event) => {
      ...
      if (importedProducts.length > 0) {
        ...
        notify(`Se importaron ${importedProducts.length} productos correctamente. Recuerda sincronizarlos con la base de datos.`);
      } else {
  ...
  const filteredProducts = useMemo(() => {
    return products.filter(p => {
      // Prevención de errores si 'name' viene vacío de la base de datos
      const safeName = p.name ? p.name.toString().toLowerCase() : '';
      const safeCode = p.code ? p.code.toString().toLowerCase() : '';
      const searchLower = searchTerm.toLowerCase();

      const matchesSearch = safeName.includes(searchLower) || safeCode.includes(searchLower);
  ...
  const handleEmployeeSubmitOrder = () => {
    ...
    // Descontar inventario de la vista actual temporalmente
    ...
    setCart([]);
    setIsCartOpenMobile(false);
    notify("¡Pedido enviado a administración!");
  };
  ...
  const handleApproveOrder = (order) => {
    ...
    // Actualizar el stock en la base de datos (Google Sheets)
    order.items.forEach(item => {
      const currentProduct = products.find(p => p.id === item.id);
      if (currentProduct) {
         // El stock real ya lo habíamos descontado visualmente, enviamos ese número final a la nube
         updateStockInSheet(item.id, currentProduct.stock);
      }
    });

    notify(`Pedido #${order.id} aprobado exitosamente.`);
  };
  ...
  const handleRejectOrder = (order) => {
    ...
    setPendingOrders(pendingOrders.filter(o => o.id !== order.id));
    notify(`Pedido #${order.id} rechazado. Stock devuelto.`, "error");
  };

  // Función para gestionar inventario localmente (no conectada a DB para escritura masiva aún)
  const saveProduct = (e) => {
    e.preventDefault();
    ...
    notify("Inventario actualizado visualmente");
  };
    ...
    <div className="flex flex-col h-full bg-[#F3EDEC]">
      <div className="p-6 bg-white border-b border-gray-100 flex flex-col gap-4 shadow-sm z-10">
        <div className="relative">
          ...
          <input 
            type="text" placeholder="Buscar productos por código o nombre..."
            ...
            <button
              ...
              className={`px-5 py-2 rounded-lg text-sm font-bold whitespace-nowrap transition-all border ${
                selectedCategory === cat 
                  ? `bg-[${COLORS.bladeBlue}] text-white border-[${COLORS.bladeBlue}] shadow-md` 
      ...
      <div className="flex-1 overflow-y-auto p-6">
        {isLoading ? (
          <div className="flex flex-col items-center justify-center h-64 text-gray-400 space-y-4">
             <div className="w-10 h-10 border-4 border-gray-200 border-t-[#035AE5] rounded-full animate-spin"></div>
             <p className="font-bold text-sm">Sincronizando inventario con la nube...</p>
          </div>
        ) : filteredProducts.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-64 text-gray-400 space-y-4">
            <Package size={48} opacity={0.2} />
            <p className="font-bold text-sm">No hay productos para mostrar en esta categoría.</p>
          </div>
        ) : (
              ...
              >
                <div className="w-full aspect-square rounded-xl bg-[#F3EDEC] mb-4 flex items-center justify-center overflow-hidden border border-gray-50 relative p-2">
                    {product.image ? (
                      ...
                      <div className="w-full h-full flex items-center justify-center rounded-lg" style={{ backgroundColor: product.color || COLORS.white }}>
                        {product.name && product.name.includes('BIC') ? <span className="text-3xl font-black text-white mix-blend-overlay">BIC</span> : <Package className="text-gray-400 opacity-50" size={32} />}
                      </div>
                    )}
    ...
    <div className="flex h-screen bg-[#F3EDEC]">
                    ...
                    <table className="w-full text-left border-collapse">
                      <thead>
                        <tr className="bg-[#F3EDEC] text-gray-500 text-xs uppercase tracking-wider">
                          ...
                          <th className="p-4 font-bold border-b border-gray-200 text-right">Total</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-gray-50">
                        {sales.map(sale => (
                          <tr key={sale.id} className="hover:bg-gray-50/50 transition-colors">
                            ...
                            <td className="p-4 text-right font-bold text-[#035AE5]">${sale.total.toFixed(2)}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
      ...
      <nav className="md:hidden fixed bottom-0 w-full bg-white border-t border-gray-200 h-20 flex items-center justify-around px-2 z-40 pb-safe shadow-[0_-5px_15px_rgba(0,0,0,0.05)]">
        {[
          ...
          <button key={item.id} onClick={() => setAdminView(item.id)} className={`flex flex-col items-center gap-1 w-1/4 transition-colors relative ${adminView === item.id ? `text-[${COLORS.bladeBlue}]` : 'text-gray-400'}`}>
            <div className={`p-1.5 rounded-lg ${adminView === item.id ? `bg-[${COLORS.bladeBlue}]/10` : ''}`}>{item.icon}</div>
          ...
          <div className="bg-white w-full max-w-lg rounded-3xl overflow-hidden shadow-2xl flex flex-col max-h-full">
            ...
            <form onSubmit={saveProduct} className="p-6 overflow-y-auto flex-1 space-y-6 hide-scrollbar">
              ...
              <div className="space-y-1.5">
                  ...
                  <select name="category" defaultValue={editingProduct?.category || 'Escritura'} className="w-full p-3.5 rounded-xl bg-white border border-gray-200 outline-none focus:border-[#035AE5] focus:ring-1 focus:ring-[#035AE5] font-bold text-black transition-all appearance-none cursor-pointer">
