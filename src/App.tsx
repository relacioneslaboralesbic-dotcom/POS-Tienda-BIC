...
const LogoBIC = ({ size = "normal", showText = true }) => (
  <div className="flex items-center gap-3">
    <img 
      src="Logo.webp" 
      alt="Logo BIC" 
      className={`${size === 'large' ? 'h-20' : 'h-10'} object-contain drop-shadow-sm`}
      onError={(e) => {
...
const App = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        notify("Error de conexión a SheetDB", "error");
        setIsLoading(false);
      }
    };
    fetchData();
  ...
  const addToCart = (product) => {
    ...
    if (item) {
      if (item.quantity >= product.stock) return notify("Límite de stock", "error");
  ...
  const handleApproveOrder = async (order) => {
    ...
    // Actualización de stock en SheetDB
    ...
    notify("Pedido autorizado y stock actualizado");
    ...
    setTimeout(() => {
      ...
      const doCapture = () => {
        window.html2canvas(element, { scale: 2, useCORS: true }).then(canvas => {
          ...
          link.download = `BIC_VALE_${order.id}.png`;
  ...
  const saveProduct = (e) => {
    ...
    setProducts(editingProduct ? products.map(p => p.id === editingProduct.id ? data : p) : [data, ...products]);
    setIsModalOpen(false);
    setEditingProduct(null);
    notify("Guardado local");
  };
      ...
      <div id="printable-ticket" className="bg-white text-black p-10 flex flex-col border-[12px] border-double border-gray-100" style={{ width: '215.9mm', minHeight: '279.4mm' }}>
        ...
        <div className="mt-8 border-t-2 border-black pt-6 flex justify-end gap-12">
          <div className="text-right space-y-1">
            <p className="text-gray-400 text-[10px] font-black uppercase">Total:</p>
        ...
        <div className="mt-auto pt-8 flex flex-col items-center">
          ...
          <p className="text-[8px] text-gray-300 font-bold mt-4 uppercase tracking-[0.3em]">Copia Controlada • BIC Saltillo</p>
        </div>
      </div>
    );
  };
  ...
  if (appMode === 'selection') {
    return (
      <div className="h-screen bg-[#F3EDEC] flex items-center justify-center p-6 relative overflow-hidden">
        <style>{globalStyles}</style>
        {/* Decoración de fondo */}
        <div className="absolute top-0 right-0 w-96 h-96 bg-orange-400/20 rounded-full blur-3xl -mr-20 -mt-20"></div>
        <div className="absolute bottom-0 left-0 w-96 h-96 bg-blue-400/20 rounded-full blur-3xl -ml-20 -mb-20"></div>
        
        <div className="bg-white p-12 rounded-[50px] shadow-2xl w-full max-w-md text-center border-b-[15px] border-[#F89332] z-10 relative">
  ...
  if (appMode.startsWith('login')) {
    return (
      <div className="h-screen bg-[#F3EDEC] flex flex-col lg:flex-row">
        <style>{globalStyles}</style>
        {/* BANNER LATERAL RESTAURADO */}
        <div className="hidden lg:flex w-1/2 bg-[#035AE5] items-center justify-center p-12 relative overflow-hidden">
          <div className="absolute inset-0 opacity-20 bg-[url('https://www.transparenttextures.com/patterns/cubes.png')]"></div>
          <div className="relative z-10 text-center text-white">
            <div className="mb-8 flex justify-center scale-150 grayscale brightness-[10]"><LogoBIC size="large" showText={false} /></div>
            <h1 className="text-5xl font-black uppercase tracking-tighter mb-4">Planta Saltillo</h1>
            <p className="text-xl font-bold opacity-80 uppercase tracking-widest italic">Suministros de Oficina e Insumos</p>
          </div>
          <div className="absolute bottom-0 right-0 p-8">
            <div className="bg-white/10 backdrop-blur-md p-6 rounded-3xl border border-white/20">
               <p className="text-xs font-black uppercase">BIC World 2024</p>
            </div>
          </div>
        </div>

        <div className="w-full lg:w-1/2 flex items-center justify-center p-8 bg-white shadow-[-20px_0_40px_rgba(0,0,0,0.05)]">
            ...
            <form onSubmit={appMode === 'login_admin' ? handleAdminLogin : handleEmployeeLogin} className="space-y-4">
              {appMode === 'login_admin' ? (
                ...
                <>
                  ...
                  <select value={empShift} onChange={e => setEmpShift(e.target.value)} className="w-full p-4 bg-[#F3EDEC] rounded-2xl outline-none font-bold text-black border-2 border-transparent focus:border-[#035AE5] transition-all appearance-none cursor-pointer">
    ...
    <div className="flex h-screen bg-[#F3EDEC]">
      ...
      {showSuccessModal && selectedOrderForTicket && (
        <div className="fixed inset-0 bg-black/60 z-[9999] flex items-center justify-center p-4 backdrop-blur-sm no-print">
          <div className="bg-white p-10 rounded-[40px] shadow-2xl max-w-md w-full text-center">
            ...
            <p className="text-gray-500 font-bold mb-8 italic text-sm px-4">Guarda tu comprobante para cualquier aclaración.</p>
            <div className="space-y-3">
              <button onClick={() => handleDownloadImage(selectedOrderForTicket)} className="w-full p-5 bg-[#035AE5] text-white rounded-3xl font-black uppercase text-xs flex justify-center gap-3 shadow-lg shadow-blue-500/30">
      ...
      <main className="flex-1 flex flex-col overflow-hidden no-print">
        {appMode === 'employee' && (
          <header className="bg-white border-b border-gray-50 p-6 flex justify-between items-center shadow-sm shrink-0 z-10">
            ...
            <div className="flex items-center gap-6">
              <div className="text-right">
                <p className="font-black uppercase text-xs text-black">{currentUser.name}</p>
        ...
        <div className="flex-1 flex overflow-hidden">
          <div className="flex-1 overflow-y-auto p-6 lg:p-10 bg-[#F3EDEC]">
            {adminView === 'dashboard' && appMode === 'admin' ? (
              <div className="space-y-6">
                <h2 className="text-2xl font-bold text-black mb-6 uppercase tracking-tighter">Indicadores</h2>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm"><TrendingUp className="text-[#035AE5] mb-4" size={24} /><p className="text-sm font-bold text-gray-400">Total Vales</p><h3 className="text-3xl font-black text-black">${sales.reduce((acc, s) => acc + s.total, 0).toFixed(2)}</h3></div>
                  <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm"><Package className="text-[#F89332] mb-4" size={24} /><p className="text-sm font-bold text-gray-400">Stock Planta</p><h3 className="text-3xl font-black text-black">{products.reduce((acc, p) => acc + p.stock, 0)}</h3></div>
                  <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm"><List className="text-[#DB054B] mb-4" size={24} /><p className="text-sm font-bold text-gray-400">Pendientes</p><h3 className="text-3xl font-black text-black">{pendingOrders.length}</h3></div>
                </div>
              </div>
               ...
               <div className="max-w-4xl mx-auto space-y-4">
                  ...
                  {sales.map(s => (
                    <div key={s.id} className="bg-white p-6 rounded-[30px] border border-gray-100 shadow-sm flex items-center justify-between group hover:shadow-xl transition-all">
                      <div><p className="text-[10px] font-black text-gray-300 uppercase">Vale: {s.id}</p><h4 className="text-lg font-black uppercase text-black">{s.empName}</h4><p className="text-xs font-bold text-gray-400">{s.date}</p></div>
                      <div className="flex items-center gap-3"><p className="text-2xl font-black text-[#035AE5] mr-4">${s.total.toFixed(2)}</p><button onClick={() => handleDownloadImage(s)} className="p-3 bg-blue-50 text-[#035AE5] rounded-xl hover:bg-blue-100 transition-colors"><Download size={20}/></button></div>
                    </div>
                  ))}
               </div>
            ) : adminView === 'orders' && appMode === 'admin' ? (
               <div className="max-w-3xl mx-auto space-y-6">
                  <h2 className="text-2xl font-black uppercase mb-8">Solicitudes Pendientes</h2>
              ...
              <div className="h-full">
                ...
                <div className="flex flex-col lg:flex-row gap-4 mb-10">
                  <div className="flex-1 relative">
                    ...
                    <input type="text" placeholder="Buscar material..." className="w-full pl-16 pr-8 py-5 rounded-[30px] bg-white border-none shadow-sm font-bold text-xl outline-none focus:ring-4 ring-blue-100 transition-all" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
                  </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 pb-20">
                  {isLoading ? (
                    <div className="col-span-full py-20 text-center text-gray-300 font-black uppercase animate-pulse italic tracking-widest">Cargando de SheetDB...</div>
                    ...
                    <div key={p.id} onClick={() => addToCart(p)} className="bg-white p-5 rounded-[40px] border border-gray-50 shadow-sm hover:shadow-2xl hover:-translate-y-2 transition-all cursor-pointer group relative">
                       <div className="aspect-square bg-gray-50 rounded-[30px] mb-4 flex items-center justify-center overflow-hidden p-6 border border-gray-50 relative">
                         ...
                         {p.stock <= 5 && p.stock > 0 && <span className="absolute top-4 right-4 bg-[#F89332] text-black px-2 py-1 rounded-lg text-[9px] font-black uppercase shadow-sm">Stock Bajo</span>}
                       </div>
          ...
          {/* CARRITO INTEGRADO */}
          {appMode === 'employee' && (
            <aside className="hidden md:flex w-[400px] bg-white border-l border-gray-100 flex flex-col shadow-[-15px_0_30px_rgba(0,0,0,0.02)] shrink-0">
              <div className="p-8 border-b border-gray-50 flex justify-between items-center bg-[#F3EDEC]">
                ...
                <span className="bg-[#035AE5] text-white px-3 py-1 rounded-full text-[10px] font-black shadow-lg uppercase">{cart.length}</span>
              </div>
              <div className="flex-1 overflow-y-auto p-6 space-y-4">
                {cart.length === 0 ? (
                  <div className="h-full flex flex-col items-center justify-center text-gray-400 font-black uppercase italic space-y-4 opacity-40">
                    <ShoppingCart size={48}/> <p className="text-sm tracking-widest">Vacío</p>
                  </div>
                ) : cart.map(item => (
              ...
              <div className="p-8 border-t border-gray-50 space-y-6">
                <div className="flex justify-between items-end">
                   <span className="text-gray-300 uppercase font-black text-[10px] tracking-widest mb-1">Subtotal</span>
        ...
        <div className="fixed inset-0 bg-black/50 z-[70] flex items-center justify-center p-4 backdrop-blur-sm no-print">
          <div className="bg-white w-full max-w-lg rounded-3xl overflow-hidden shadow-2xl flex flex-col max-h-full">
            <div className="flex justify-between items-center p-6 border-b border-gray-100">
              <h2 className="text-xl font-bold text-black uppercase tracking-tight">{editingProduct ? 'Editar Material' : 'Alta de Material'}</h2>
